...
<div class="flex-1 p-4">
    <div class="block text-gray-300 font-semibold text-3xl">Due Today</div>
                    ...
                    <?php foreach ($todays as $today) : ?>
                        ...
                        <li class="p-4 rounded-lg bg-zinc-950 my-4 flex flex-row justify-between">
                            ...
                            <div class="flex flex-col">
                                ...
                                <!-- Hidden favorite -->
                                <input type="hidden" name="taskFavorite" value="<?= htmlspecialchars($today['is_favorite']) ?>">

                                <!-- Hidden last updated -->
                                <input type="hidden" name="taskUpdateAt" value="<?= htmlspecialchars($today['updated_at']) ?>">

                                <!-- Title -->
                                <h3 class="text-lg font-semibold text-stone-200 hover:underline hover:cursor-pointer hover:text-red-800" id="taskTitle"><?= htmlspecialchars($today['title']) ?></h3>

                                <!-- Description -->
                                <p class="mt-2 ml-5 font-medium text-gray-500" id="taskDescription"><?= htmlspecialchars($today['description']) ?></p>

                                <!-- Due Date -->
                                <div>
                                    <i class="text-white fa-solid fa-calendar-day ml-5 mt-3 mr-2"></i>
                                    <span class="text-white " id="taskDueDate"><?= htmlspecialchars($today['due_date']) ?></span>
                                </div>
